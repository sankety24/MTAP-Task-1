...
const fetchData = (page) => {
    axios
      .get(`https://stock-server.onrender.com/products?category=garlands&_limit=12&_page=${page}`)
      .then((e) => {
        setLoading(true);
        console.log(e);
        setClothes(e.data);
              ...
              <div className="clothData">
                {cloth.length>0 && cloth.map((data)=>(
                    <div key ={data.id} className="cloth">
                       <Link to={`/gardens/${data.id}`}> <div className="image"> <img src={data.image} alt=""/></div> </Link>
